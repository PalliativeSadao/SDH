const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbyQIbm1vl8AmYxGe5qBV0erTS15WpLiO2VxEUt9OBqiInmsMH-pEW7GwVkNZy_plFVu/exec'; // *** อย่าลืมวาง URL ใหม่ ***
...
function toggleDeathPlace() {
    ...
    if (status === 'Death') {
        ...
        container.classList.add('d-none'); document.getElementById('death_place').value = ''; document.getElementById('withdraw_et').checked = false; 
        const dd = document.getElementById('discharge_date'); dd.value = ''; if(dd._flatpickr) dd._flatpickr.clear();
    }
}
function revertToAlive() { document.getElementById('status_alive').checked = true; toggleDeathPlace(); Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'เปลี่ยนสถานะเป็น คงพยาบาล', showConfirmButton: false, timer: 2000 }); }
function toggleLWDate() { const isChecked = document.getElementById('lw_checkbox').checked; const dateContainer = document.getElementById('lw_date_container'); if(isChecked) dateContainer.classList.remove('d-none'); else { dateContainer.classList.add('d-none'); document.getElementById('lw_date').value = ''; } }
function updateEditHeader() { if(!editingPatient) return; document.getElementById('editInfoName').innerText = document.getElementById('fullname').value; document.getElementById('editInfoCurrentType').innerText = "Now: " + document.getElementById('admitType').value; }

// --- Submit ---
function handleFormSubmit(e) {
  e.preventDefault();
  
  // ตรวจสอบค่าว่าง (แก้บัคแจ้งเตือนไม่เด้งบนคอมพิวเตอร์)
  const hnVal = document.getElementById('hn').value.trim();
  const nameVal = document.getElementById('fullname').value.trim();
  if(!hnVal || hnVal.length < 1) { Swal.fire('ข้อมูลไม่ครบ', 'กรุณากรอกเลข HN', 'warning'); document.getElementById('hn').focus(); return; }
  if(!nameVal) { Swal.fire('ข้อมูลไม่ครบ', 'กรุณากรอกชื่อ - สกุล', 'warning'); document.getElementById('fullname').focus(); return; }
  ...
  acp['maker'] = document.getElementById('acp_maker').value; acp['date'] = dateThToGregorian(document.getElementById('acp_date').value); 
  ...
  const formData = {
    ...
    hn: hnVal, name: nameVal, gender: document.getElementById('gender').value,
    ...
    nextVisitDate: dateThToGregorian(document.getElementById('next_visit_date').value), nextVisitType: document.getElementById('next_visit_type').value,
...
function openNewRegistration() {
  resetForm(); editingPatient = null; document.getElementById('editInfoBar').classList.add('d-none');
  document.getElementById('formTitle').innerHTML = '<i class="fas fa-user-plus"></i> ลงทะเบียนรายใหม่';
  document.getElementById('btnViewHistory').classList.add('d-none'); document.getElementById('btnDeletePatient').classList.add('d-none'); 
  document.getElementById('hn').readOnly = false; showPage('register');
}
...
function openEditRegistration(hn) {
  ...
  document.getElementById('editInfoName').innerText = p.name; document.getElementById('editInfoFirstType').innerText = "First: " + (p.type_admit || '-'); document.getElementById('editInfoCurrentType').innerText = "Now: " + (p.visit_type || 'OPD');
  document.getElementById('formTitle').innerHTML = `<i class="fas fa-edit"></i> แก้ไขข้อมูล`;
  document.getElementById('btnViewHistory').classList.remove('d-none'); document.getElementById('btnDeletePatient').classList.remove('d-none'); 
  ...
  if(p.address) { ['house','moo','road','tumbon','amphoe','province'].forEach(k=>{ const key = (k==='tumbon')?'sub':(k==='amphoe')?'dist':(k==='province')?'prov':k; if(document.getElementById('addr_'+k)) document.getElementById('addr_'+k).value = p.address[key]||''; }); }
  ...
  if(p.livingWill) { const isMade = p.livingWill.status === 'Made'; document.getElementById('lw_checkbox').checked = isMade; toggleLWDate(); if(isMade) document.getElementById('lw_date').value = gregorianToDateTh(p.livingWill.date); }
  if(p.acp) { Object.keys(p.acp).forEach(k=>{ if(k === 'maker') document.getElementById('acp_maker').value = p.acp[k]; else if(k === 'date') document.getElementById('acp_date').value = gregorianToDateTh(p.acp[k]); else { const r=document.getElementsByName('acp_'+k); r.forEach(el=>{ if(el.value===p.acp[k]) el.checked=true; }); } }); }
  
  document.getElementsByName('pt_status').forEach(el=>{ if(el.value===p.status) el.checked=true; });
  if(p.status === 'Death') { toggleDeathPlace(); document.getElementById('death_place').value = p.death_place || ''; document.getElementById('withdraw_et').checked = (p.withdraw_et === 'Yes'); }
...
function resetForm() {
  ...
  document.querySelectorAll('input[type=checkbox], input[type=radio]').forEach(el => { if(el.name !== 'pt_status') el.checked = false; });
  toggleLWDate(); toggleDeathPlace(); document.getElementById('age_display').value = '-';
...
function loadHistoryToEdit(index) {
    ...
    if(h.livingWill) { const isMade = h.livingWill.status === 'Made'; document.getElementById('lw_checkbox').checked = isMade; toggleLWDate(); if(isMade) document.getElementById('lw_date').value = gregorianToDateTh(h.livingWill.date); }
...
function renderMedsList(){document.getElementById('medList').innerHTML=currentMeds.map((m,i)=>`<li class="list-group-item d-flex justify-content-between p-2 small">${m.name} (${m.dose}) <button type="button" class="btn btn-sm btn-outline-danger border-0" onclick="currentMeds.splice(${i},1);renderMedsList()"><i class="fas fa-times"></i></button></li>`).join('');}

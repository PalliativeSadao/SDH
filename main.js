...
function updateEditHeader() { 
    ...
    if(nm && at) {
        document.getElementById('editInfoName').innerText = nm.value; 
        document.getElementById('editInfoCurrentType').innerText = "Now: " + at.value; 
    }
}
...
function openEditRegistration(hn) {
  ...
  if(p.livingWill) { const isMade = p.livingWill.status === 'Made'; const lwc = document.getElementById('lw_checkbox'); if(lwc) { lwc.checked = isMade; toggleLWDate(); if(isMade) document.getElementById('lw_date').value = gregorianToDateTh(p.livingWill.date); } }
...
// *** แก้ไขฟังก์ชัน resetForm เพื่อดักจับ Object Null ได้อย่างปลอดภัย ***
function resetForm() {
  ...
  // คืนค่าสถานะกลับเป็น Alive แบบป้องกัน Error
...
function loadHistoryToEdit(index) {
    ...
    const hModal = document.getElementById('historyModal');
    if(hModal) {
        const modal = bootstrap.Modal.getInstance(hModal);
        if(modal) modal.hide();
    }
    resetForm();
...
// ... Helpers (Buttons with type="button" are fixed in HTML, JS generator strings below) ...
